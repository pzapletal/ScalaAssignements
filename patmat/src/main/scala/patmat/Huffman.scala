...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.isEmpty) List[CodeTree]()
      ...
      val combined = Fork(tree1, tree2, 
          chars(tree1) ::: chars(tree2), weight(tree1) + weight(tree2))
      ...
      if (text isEmpty) List[Bit]()
        ...
        case Fork(left, right, _, _) =>
          if (chars(left).contains(text head)) {
            0 :: encodeHelp(left, text)
          } else {
            1 :: encodeHelp(right, text)
          }
      }
    }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertInner(sub_tree: CodeTree): CodeTable = {
      sub_tree match {
        case Leaf(char, _) => List((char, encode(tree)(List(char))))
        case Fork(left, right, _, _) => convertInner(left) ::: convertInner(right)
      }
    }
    convertInner(tree)
  }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    val bits = codeBits(table)_
    def quickEncodeHelp(text: List[Char]): List[Bit] = {
      if (text isEmpty) List[Bit]()
      else {
        bits(text head) ::: quickEncodeHelp(text tail)
      }
    }
